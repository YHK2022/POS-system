...
class AccountsController extends Controller
{
    ...
    public function balanceSheet()
    {
        ...
        if ($role->hasPermissionTo('balance-sheet')) {
            ...
            foreach ($lims_account_list as $account) {
                $payment_recieved = Payment::whereNotNull('sale_id')->where('account_id', $account->id)
                ->where('company_id', auth()->user()->company_id)
                ->sum('amount');
                $payment_sent = Payment::whereNotNull('purchase_id')->where('account_id', $account->id)
                ->where('company_id', auth()->user()->company_id)
                ->sum('amount');
                $returns = DB::table('returns')->where('account_id', $account->id)
                ->where('company_id', auth()->user()->company_id)
                ->sum('grand_total');
                $return_purchase = DB::table('return_purchases')->where('account_id', $account->id)
                ->where('company_id', auth()->user()->company_id)
                ->sum('grand_total');
                $expenses = DB::table('expenses')->where('account_id', $account->id)
                ->where('company_id', auth()->user()->company_id)
                ->sum('amount');
                $payrolls = DB::table('payrolls')->where('account_id', $account->id)
                ->where('company_id', auth()->user()->company_id)
                ->sum('amount');
    ...
    public function accountStatement(Request $request)
    {
        ...
        if ($data['type'] == '0' || $data['type'] == '1') {
            ...
            $expense_list = Expense::where('account_id', $data['account_id'])->whereDate('created_at', '>=', $data['start_date'])->whereDate('created_at', '<=', $data['end_date'])
            ->where('company_id', auth()->user()->company_id)
            ->get();

            $return_list = Returns::where('account_id', $data['account_id'])->whereDate('created_at', '>=', $data['start_date'])->whereDate('created_at', '<=', $data['end_date'])
            ->where('company_id', auth()->user()->company_id)
            ->get();

            $purchase_return_list = ReturnPurchase::where('account_id', $data['account_id'])->whereDate('created_at', '>=', $data['start_date'])->whereDate('created_at', '<=', $data['end_date'])
            ->where('company_id', auth()->user()->company_id)
            ->get();

            $payroll_list = Payroll::where('account_id', $data['account_id'])->whereDate('created_at', '>=', $data['start_date'])->whereDate('created_at', '<=', $data['end_date'])
            ->where('company_id', auth()->user()->company_id)
            ->get();
