...
class SaleController extends Controller
{
    ...
    {      
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                $lims_sale_all = Sale::orderBy('id', 'desc')->where('user_id', Auth::id())
                ->where('company_id', $company_id)
                ->get();
            else
                $lims_sale_all = Sale::orderBy('id', 'desc')
                ->where('company_id', $company_id)
                ->get();

            $lims_gift_card_list = GiftCard::where("is_active", true)->get();
            $lims_pos_setting_data = PosSetting::latest()
    ...
    public function saleData(Request $request)
    {
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
            ...
            $totalData = Sale::where('company_id', $company_id)
            ->where('company_id', auth()->user()->company_id)
            ->count();
    ...
    public function store(Request $request)
    {
      ...
      $data['company_id'] = $company_id;
            ...
            if($sale_unit[$i] != 'n/a') {
                ...
                if($data['sale_status'] == 1){
                    ...
                    if($lims_product_data->is_variant) {
                        ...
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant($id, $lims_product_variant_data->variant_id, $data['warehouse_id'])
                        ->where('company_id', auth()->user()->company_id)
                        ->first();
                    }
                    else {
                        $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($id, $data['warehouse_id'])
                        ->where('company_id', auth()->user()->company_id)
                        ->first();
                    }
        ...
        if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            $lims_payment_data->company_id = auth()->user()->company_id;
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            ->get();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_product_list = Product::select('id', 'name', 'code', 'image')->ActiveFeatured()->whereNull('is_variant')
            ->where('company_id', auth()->user()->company_id)
            ->get();
            ...
            $lims_product_list_with_variant = Product::select('id', 'name', 'code', 'image')->ActiveFeatured()->whereNotNull('is_variant')
            ->where('company_id', auth()->user()->company_id)
            ->get();
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                $recent_sale = Sale::where('sale_status', 1)->orderBy('id', 'desc')->take(10)
                ->where('company_id', auth()->user()->company_id)
                ->get();
                $recent_draft = Sale::where('sale_status', 3)->orderBy('id', 'desc')->take(10)
                ->where('company_id', auth()->user()->company_id)
                ->get();
            }
    ...
    public function genInvoice($id)
    {
        ...
        $lims_customer_data = Customer::find($lims_sale_data->customer_id);
        $lims_payment_data = Payment::where('sale_id', $id)
        ->where('company_id', auth()->user()->company_id)
        ->get();
    ...
    public function addPayment(Request $request)
    {
        ...
        $lims_payment_data = Payment::latest()
        ->where('company_id', auth()->user()->company_id)
        ->first();
        $data['payment_id'] = $lims_payment_data->id;
    ...
    public function todayProfit($warehouse_id)
    {
        ...
        if($warehouse_id == 0)
            ...
            $data['expense_amount'] = Expense::where('warehouse_id', $warehouse_id)->whereDate('created_at', date("Y-m-d"))
            ->where('company_id', auth()->user()->company_id)
            ->sum('amount');
