...
class CashRegisterController extends Controller
{
	public function index()
	{
		if(Auth::user()->role_id <= 2) {
			$lims_cash_register_all = CashRegister::with('user', 'warehouse')
	...
	public function getDetails($id)
	{
		...
		$data['total_sale_return'] = Returns::where('cash_register_id', $cash_register_data->id)
		->where('company_id', auth()->user()->company_id)
		->sum('grand_total');
		$data['total_expense'] = Expense::where('cash_register_id', $cash_register_data->id)
		->where('company_id', auth()->user()->company_id)
		->sum('amount');
