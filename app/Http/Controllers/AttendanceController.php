...
class AttendanceController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('attendance')) {
            $lims_employee_list = Employee::where('is_active', true)
            ...
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own')
                $lims_attendance_all = Attendance::orderBy('id', 'desc')->where('user_id', Auth::id())
                ->where('company_id', auth()->user()->company_id)
                ->get();
            else
                $lims_attendance_all = Attendance::orderBy('id', 'desc')
                ->where('company_id', auth()->user()->company_id)
                ->get();
    ...
    public function store(Request $request)
    {
        ...
        foreach ($employee_id as $id) {
            ...
            $lims_attendance_data = Attendance::whereDate('date', $data['date'])->where('employee_id', $id)
            ->where('company_id', auth()->user()->company_id)
            ->first();
