...
class SettingController extends Controller
{
    ...
    {
        $lims_general_setting_data = GeneralSetting::latest()
    ...
    public function generalSettingStore(Request $request)
    {
        ...
        $general_setting = GeneralSetting::latest()
        ->where('company_id', auth()->user()->company_id)
        ->first();
        // $general_setting->id = 1;
        $general_setting->company_id = Auth()->user()->company_id;
    ...
    {
        $lims_hrm_setting_data = HrmSetting::latest()
        ->where('company_id', auth()->user()->company_id)
        ->first();
    ...
    {
        $data = $request->all();
        $lims_hrm_setting_data = HrmSetting::firstOrNew(['company_id' => auth()->user()->company_id]);
        $lims_hrm_setting_data->checkin = $data['checkin'];
        $lims_hrm_setting_data->checkout = $data['checkout'];
        $lims_hrm_setting_data->company_id = auth()->user()->company_id;
    ...
    public function posSettingStore(Request $request)
    {
        ...
        $pos_setting = PosSetting::firstOrNew(['company_id' => auth()->user()->company_id]);
        // $pos_setting->id = 1;
        $pos_setting->customer_id = $data['customer_id'];
        $pos_setting->company_id = auth()->user()->company_id;
