...
class PurchaseController extends Controller
{
    public function index()
    {
        ...
        if ($role->hasPermissionTo('purchases-index')) {
            if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                $lims_purchase_list = Purchase::orderBy('id', 'desc')->where('user_id', Auth::id())
                ->where('company_id', auth()->user()->company_id)
                ->get();
            } else {
                $lims_purchase_list = Purchase::orderBy('id', 'desc')
                ->where('company_id', auth()->user()->company_id)
                ->get();
            }
    ...
    public function purchaseData(Request $request)
    {
        ...
        if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            ...
            $totalData = Purchase::where('company_id', auth()->user()->company_id)->count();
        }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($product_id as $i => $id) {
            ...
            if ($lims_product_warehouse_data) {
                ...
                $lims_product_warehouse_data->company_id = auth()->user()->company_id;
            ...
            $product_purchase['company_id'] = auth()->user()->company_id;
