...
class ReportController extends Controller
{
    ...
    public function warehouseStock()
    {
        ...
        if ($role->hasPermissionTo('warehouse-stock-report')) {
            ...
            $total_qty = Product::where('is_active', true)
                ->where('company_id', auth()->user()->company_id)
                ->sum('qty');
            $total_price = DB::table('products')->where('is_active', true)
                ->where('company_id', auth()->user()->company_id)
                ->sum(DB::raw('price * qty'));
            $total_cost = DB::table('products')->where('is_active', true)
                ->where('company_id', auth()->user()->company_id)
                ->sum(DB::raw('cost * qty'));
    ...
    public function dailySale($year, $month)
    {
        ...
        if ($role->hasPermissionTo('daily-sale')) {
            ...
            while ($start <= $number_of_day) {
                ...
                $sale_data = Sale::whereDate('created_at', $date)->selectRaw(implode(',', $query1))
                    ->where('company_id', auth()->user()->company_id)
                    ->get();
    ...
    public function dailySaleByWarehouse(Request $request, $year, $month)
    {
        ...
        while ($start <= $number_of_day) {
            ...
            $sale_data = Sale::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', $date)->selectRaw(implode(',', $query1))
                ->where('company_id', auth()->user()->company_id)
                ->get();
            $total_discount[$start] = $sale_data[0]->total_discount;
            $order_discount[$start] = $sale_data[0]->order_discount;
    ...
    public function dailyPurchase($year, $month)
    {
        ...
        if ($role->hasPermissionTo('daily-purchase')) {
            ...
            while ($start <= $number_of_day) {
                ...
                $purchase_data = Purchase::whereDate('created_at', $date)->selectRaw(implode(',', $query1))
                    ->where('company_id', auth()->user()->company_id)
                    ->get();
    ...
    public function dailyPurchaseByWarehouse(Request $request, $year, $month)
    {
        ...
        while ($start <= $number_of_day) {
            ...
            $purchase_data = Purchase::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', $date)->selectRaw(implode(',', $query1))
                ->where('company_id', auth()->user()->company_id)
                ->get();
            $total_discount[$start] = $purchase_data[0]->total_discount;
            $order_discount[$start] = $purchase_data[0]->order_discount;
    ...
    public function monthlySale($year)
    {
        ...
        if ($role->hasPermissionTo('monthly-sale')) {
            ...
            while ($start <= $end) {
                ...
                $temp_total = Sale::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                    ->where('company_id', auth()->user()->company_id)
                    ->sum('grand_total');
    ...
    public function monthlySaleByWarehouse(Request $request, $year)
    {
        ...
        while ($start <= $end) {
            ...
            $temp_total_discount = Sale::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                ->where('company_id', auth()->user()->company_id)
                ->sum('total_discount');
            $total_discount[] = number_format((float) $temp_total_discount, 2, '.', '');

            $temp_order_discount = Sale::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                ->where('company_id', auth()->user()->company_id)
                ->sum('order_discount');
            $order_discount[] = number_format((float) $temp_order_discount, 2, '.', '');

            $temp_total_tax = Sale::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                ->where('company_id', auth()->user()->company_id)
                ->sum('total_tax');
            $total_tax[] = number_format((float) $temp_total_tax, 2, '.', '');

            $temp_order_tax = Sale::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                ->where('company_id', auth()->user()->company_id)
                ->sum('order_tax');
            $order_tax[] = number_format((float) $temp_order_tax, 2, '.', '');

            $temp_shipping_cost = Sale::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                ->where('company_id', auth()->user()->company_id)
                ->sum('shipping_cost');
            $shipping_cost[] = number_format((float) $temp_shipping_cost, 2, '.', '');

            $temp_total = Sale::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                ->where('company_id', auth()->user()->company_id)
                ->sum('grand_total');
            $total[] = number_format((float) $temp_total, 2, '.', '');
            $start = strtotime("+1 month", $start);
    ...
    public function monthlyPurchase($year)
    {
        ...
        if ($role->hasPermissionTo('monthly-purchase')) {
            ...
            while ($start <= $end) {
                ...
                $purchase_data = Purchase::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query1))
                    ->where('company_id', auth()->user()->company_id)
                    ->get();
    ...
    public function monthlyPurchaseByWarehouse(Request $request, $year)
    {
        ...
        while ($start <= $end) {
            ...
            $purchase_data = Purchase::where('warehouse_id', $data['warehouse_id'])->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query1))
                ->where('company_id', auth()->user()->company_id)
                ->get();
    ...
    public function bestSeller()
    {
        ...
        if ($role->hasPermissionTo('best-seller')) {
            ...
            while ($start <= $end) {
                ...
                $best_selling_qty = Product_Sale::select(DB::raw('product_id, sum(qty) as sold_qty'))->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->groupBy('product_id')->orderBy('sold_qty', 'desc')->take(1)
                ->where('company_id', auth()->user()->company_id)
                ->get();
    ...
    public function bestSellerByWarehouse(Request $request)
    {
        ...
        while ($start <= $end) {
            ...
            $best_selling_qty = DB::table('sales')
                ->join('product_sales', 'sales.id', '=', 'product_sales.sale_id')->select(DB::raw('product_sales.product_id, sum(product_sales.qty) as sold_qty'))->where('sales.warehouse_id', $data['warehouse_id'])->whereDate('sales.created_at', '>=', $start_date)->whereDate('sales.created_at', '<=', $end_date)->groupBy('product_id')->orderBy('sold_qty', 'desc')->take(1)
                ->where('company_id', auth()->user()->company_id)
                ->get();
    ...
    public function profitLoss(Request $request)
    {
        ...
        $purchase = Purchase::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query1))
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $total_purchase = Purchase::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->count();
        $sale = Sale::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query1))
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $total_sale = Sale::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->count();
        $return = Returns::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query2))
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $total_return = Returns::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->count();
        $purchase_return = ReturnPurchase::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query2))
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $total_purchase_return = ReturnPurchase::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->count();
        $expense = Expense::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->sum('amount');
        $total_expense = Expense::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->count();
        $payroll = Payroll::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->sum('amount');
        $total_payroll = Payroll::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->count();
        $total_item = DB::table('product_warehouse')
            ->join('products', 'product_warehouse.product_id', '=', 'products.id')
        ...
        $payment_recieved_number = DB::table('payments')->whereNotNull('sale_id')->whereDate('created_at', '>=', $start_date)
            ...
            ->where('company_id', auth()->user()->company_id)
            ->count();
        $payment_recieved = DB::table('payments')->whereNotNull('sale_id')->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
        ->where('company_id', auth()->user()->company_id)
        ->sum('payments.amount');
        ...
        $payment_sent_number = DB::table('payments')->whereNotNull('purchase_id')->whereDate('created_at', '>=', $start_date)
            ...
            ->where('company_id', auth()->user()->company_id)
            ->count();
        $payment_sent = DB::table('payments')->whereNotNull('purchase_id')->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
        ->where('company_id', auth()->user()->company_id)
        ->sum('payments.amount');
        ...
        foreach ($lims_warehouse_all as $warehouse) {
            ...
            $warehouse_sale[] = Sale::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query2))
                ->where('company_id', auth()->user()->company_id)
                ->get();
            $warehouse_purchase[] = Purchase::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query2))
                ->where('company_id', auth()->user()->company_id)
                ->get();
            $warehouse_return[] = Returns::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query2))
                ->where('company_id', auth()->user()->company_id)
                ->get();
            $warehouse_purchase_return[] = ReturnPurchase::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->selectRaw(implode(',', $query2))
                ->where('company_id', auth()->user()->company_id)
                ->get();
            $warehouse_expense[] = Expense::where('warehouse_id', $warehouse->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                ->where('company_id', auth()->user()->company_id)
                ->sum('amount');
        }
    ...
    public function productReport(Request $request)
    {
        ...
        $product_name = [];
        $product_qty = [];
        $lims_product_all = Product::select('id', 'name', 'qty', 'is_variant')->where('is_active', true)
            ->where('company_id', auth()->user()->company_id)
            ->get();
        foreach ($lims_product_all as $product) {
            ...
            if ($warehouse_id == 0) {
                if ($product->is_variant) {
                    $variant_id_all = ProductPurchase::distinct('variant_id')->where('product_id', $product->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                    ->where('company_id', auth()->user()->company_id)
                    ->pluck('variant_id');
                } else {
                    $lims_product_purchase_data = ProductPurchase::where('product_id', $product->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                    ->where('company_id', auth()->user()->company_id)
                    ->first();
                }
            ...
            if ($lims_product_purchase_data) {
                ...
                if ($warehouse_id == 0) {
                    if ($product->is_variant) {
                        $variant_id_all = Product_Sale::distinct('variant_id')->where('product_id', $product->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                        ->where('company_id', auth()->user()->company_id)
                        ->pluck('variant_id');
                        //return $lims_product_sale_data;
                    } else {
                        $lims_product_sale_data = Product_Sale::where('product_id', $product->id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
                        ->where('company_id', auth()->user()->company_id)
                        ->first();
                    }
    ...
    public function paymentReportByDate(Request $request)
    {
        ...
        $lims_payment_data = Payment::whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
        ->where('company_id', auth()->user()->company_id)
        ->get();
    ...
    public function warehouseReport(Request $request)
    {
        ...
        $lims_purchase_data = Purchase::where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_sale_data = Sale::with('customer')->where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_quotation_data = Quotation::with('customer')->where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_return_data = Returns::with('customer', 'biller')->where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_expense_data = Expense::with('expenseCategory')->where('warehouse_id', $warehouse_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        ...
        foreach ($lims_purchase_data as $key => $purchase) {
            $lims_product_purchase_data[$key] = ProductPurchase::where('purchase_id', $purchase->id)
            ->where('company_id', auth()->user()->company_id)
            ->get();
        }
        ...
        foreach ($lims_quotation_data as $key => $quotation) {
            $lims_product_quotation_data[$key] = ProductQuotation::where('quotation_id', $quotation->id)
            ->where('company_id', auth()->user()->company_id)
            ->get();
        }
        foreach ($lims_return_data as $key => $return) {
            $lims_product_return_data[$key] = ProductReturn::where('return_id', $return->id)
            // ->where('company_id', auth()->user()->company_id)
            ->get();
        }
        $lims_warehouse_list = Warehouse::where('is_active', true)
    ...
    public function userReport(Request $request)
    {
        ...
        $lims_sale_data = Sale::with('customer', 'warehouse')->where('user_id', $user_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_purchase_data = Purchase::with('warehouse')->where('user_id', $user_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_quotation_data = Quotation::with('customer', 'warehouse')->where('user_id', $user_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_transfer_data = Transfer::with('fromWarehouse', 'toWarehouse')->where('user_id', $user_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_payment_data = DB::table('payments')
            ...
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_expense_data = Expense::with('warehouse', 'expenseCategory')->where('user_id', $user_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_payroll_data = Payroll::with('employee')->where('user_id', $user_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        ...
        foreach ($lims_transfer_data as $key => $transfer) {
            $lims_product_transfer_data[$key] = ProductTransfer::where('transfer_id', $transfer->id)
            ->where('company_id', auth()->user()->company_id)
            ->get();
        }
    ...
    public function customerReport(Request $request)
    {
        ...
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        $lims_sale_data = Sale::with('warehouse')->where('customer_id', $customer_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_quotation_data = Quotation::with('warehouse')->where('customer_id', $customer_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_return_data = Returns::with('warehouse', 'biller')->where('customer_id', $customer_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_payment_data = DB::table('payments')
    ...
    public function supplierReport(Request $request)
    {
        ...
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        $lims_purchase_data = Purchase::with('warehouse')->where('supplier_id', $supplier_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
        $lims_quotation_data = Quotation::with('warehouse', 'customer')->where('supplier_id', $supplier_id)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)->orderBy('created_at', 'desc')
            ->where('company_id', auth()->user()->company_id)
            ->get();
    ...
    public function dueReportByDate(Request $request)
    {
        ...
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        $lims_sale_data = Sale::where('payment_status', '!=', 4)->whereDate('created_at', '>=', $start_date)->whereDate('created_at', '<=', $end_date)
            ->where('company_id', auth()->user()->company_id)
            ->get();
