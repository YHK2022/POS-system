...
class ReturnController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('returns-index')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own')
                ...
                ->orderBy('id', 'desc')->orderBy('id', 'desc')->where('user_id', Auth::id())
                ->where('company_id', auth()->user()->company_id)
                ->get();
            else
                $lims_return_all = Returns::with('biller', 'customer', 'warehouse', 'user')->orderBy('id', 'desc')
                ->where('company_id', auth()->user()->company_id)
                ->get();
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('returns-add')){
            ...
            if(Auth::user()->role_id <= 2) {
                ...
                $lims_warehouse_list = Warehouse::where('id',Auth::user()->warehouse_id)
                ->where('company_id', auth()->user()->company_id)
                ->get();
                $lims_biller_list = Biller::where('id', Auth::user()->biller_id)
                ->where('company_id', auth()->user()->company_id)
                ->get();
            }
    ...
    public function getProduct($id)
    {
        ...
        ])->whereNull('product_warehouse.variant_id')->select('product_warehouse.*')
        ...
        ->where([
            ['products.is_active', true],
            ['product_warehouse.warehouse_id', $id],
        ])->whereNotNull('product_warehouse.variant_id')->select('product_warehouse.*')
        ->where('products.company_id', auth()->user()->company_id)
        ->get();
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse) 
        {
            ...
            $lims_product_data = Product::select('code', 'name', 'type')
            ->where('company_id', auth()->user()->company_id)
            ->find($product_warehouse->product_id);
        ...
        {
            $product_qty[] = $product_warehouse->qty;
            $lims_product_data = Product::select('name', 'type')
            ->where('company_id', auth()->user()->company_id)
            ->find($product_warehouse->product_id);
        ...
        $lims_product_data = Product::select('code', 'name', 'type')->where('is_active', true)->whereNotIn('type', ['standard'])
        ->where('company_id', auth()->user()->company_id)
        ->get();
